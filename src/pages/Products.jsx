    ...
    >
      <div className="relative overflow-hidden rounded-xl bg-gray-100 aspect-[4/5] mb-4 shadow-sm hover:shadow-lg transition-all duration-300">
        ...
        <div className="absolute top-3 left-3 flex flex-col space-y-2">
          {product.condition === 'new' && (
            <span className="bg-emerald-500 text-white px-3 py-1 text-xs font-medium rounded-full shadow-lg">NEW</span>
          )}
          {product.is_sold && (
...
const Products = () => {
  ...
  const filteredProducts = useMemo(() => {
    let filtered = products.filter(product => {
      ...
      else if (priceRange === '100k - 300k') matchesPrice = product.price >= 100000 && product.price <= 300000;
  ...
  const priceOptions = ['All', 'Under 100k', '100k - 300k', '300k - 500k', 'Over 500k'];
          ...
          <main className="flex-1">
            ...
            <div className="mb-6 flex items-center justify-between">
              <p className="text-gray-600">
                Showing <span className="font-semibold text-black">{filteredProducts.length}</span> of <span className="font-semibold text-black">{products.length}</span> products
              </p>
